...
if [[ -n "${AICAGE_PACKAGE_PIP_INSTALL:-}" ]]; then
  pip_packages[0]="pip==${AICAGE_PACKAGE_PIP_INSTALL}"
fi
if [[ -n "${AICAGE_PACKAGE_SETUPTOOLS_INSTALL:-}" ]]; then
  pip_packages[1]="setuptools==${AICAGE_PACKAGE_SETUPTOOLS_INSTALL}"
fi
if [[ -n "${AICAGE_PACKAGE_WHEEL_INSTALL:-}" ]]; then
  pip_packages[2]="wheel==${AICAGE_PACKAGE_WHEEL_INSTALL}"
fi
...
if ! command -v pipx >/dev/null 2>&1; then
  ...
  if [[ -n "${AICAGE_PACKAGE_PIPX_INSTALL:-}" ]]; then
    pipx_package="pipx==${AICAGE_PACKAGE_PIPX_INSTALL}"
  fi
...
if [[ -n "${AICAGE_PACKAGE_UV_INSTALL:-}" ]]; then
  uv_package="uv==${AICAGE_PACKAGE_UV_INSTALL}"
fi
