...
# - AICAGE_USERNAME: target runtime username
...
AICAGE_USERNAME="${AICAGE_USERNAME:-root}"
...
setup_user_and_group() {
  ...
  existing_user_uid="$(getent passwd "${AICAGE_USERNAME}" | cut -d: -f3 || true)"
  if [[ -n "${existing_user_uid}" && "${existing_user_uid}" != "${AICAGE_UID}" ]]; then
    userdel -r "${AICAGE_USERNAME}" 2>/dev/null
  fi

  existing_user_name="$(getent passwd "${AICAGE_UID}" | cut -d: -f1 || true)"
  if [[ -n "${existing_user_name}" && "${existing_user_name}" != "${AICAGE_USERNAME}" ]]; then
  ...
  if [[ -n "${existing_group_name}" ]]; then
    ...
    groupadd -g "${AICAGE_GID}" "${AICAGE_USERNAME}"
    primary_group_name="${AICAGE_USERNAME}"
  fi

  if [[ -d "${AICAGE_HOME}" ]]; then
    useradd --no-create-home -u "${AICAGE_UID}" -g "${primary_group_name}" -d "${AICAGE_HOME}" -s /bin/bash "${AICAGE_USERNAME}"
  else
    useradd --create-home -u "${AICAGE_UID}" -g "${primary_group_name}" -d "${AICAGE_HOME}" -s /bin/bash "${AICAGE_USERNAME}"
  fi
...
setup_docker_group() {
  ...
  if [[ -n "${docker_gid_group}" ]]; then
    usermod -aG "${docker_gid_group}" "${AICAGE_USERNAME}"
  fi
}
...
mirror_windows_home_mounts_to_root
apply_timezone
set_target_env "${AICAGE_HOME}" "${AICAGE_USERNAME}"
