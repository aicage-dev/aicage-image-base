...
| `AICAGE_USERNAME` | No | Target runtime username | `root` |
...
2. Derive `AICAGE_UID`, `AICAGE_GID`, `AICAGE_USERNAME`, `AICAGE_HOME`.
...
`if [[ "${AICAGE_USERNAME}" == "root" ]]` branch.
...
is `AICAGE_UID=0`, `AICAGE_GID=0`, `AICAGE_USERNAME=root`.
...
`AICAGE_UID`, `AICAGE_GID`, and `AICAGE_USERNAME` describe runtime identity.
...
| --- | --- | --- |
| Non-root runtime user | `AICAGE_UID`, `AICAGE_GID`, `AICAGE_USERNAME`, `AICAGE_HOME` | Create a matching user, repair access, then run through `gosu`. |
