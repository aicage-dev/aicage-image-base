...
base="$1"
package_list="${ROOT_DIR}/packages/${base}.yml"
...
output_dir="${ROOT_DIR}/package-versions/${distro}"
mkdir -p "${output_dir}"
...
jq \
  ...
  --arg package_list "packages/${base}.yml" \
  --arg base_yml "bases/${base}/base.yml" \
  '
    {
      "distro": $distro,
