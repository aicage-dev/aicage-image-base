...
check_apk_specs() {
  ...
  while IFS= read -r spec; do
    ...
    package="${spec%%=*}"
    expected="${spec#*=}"
    actual="$(apk info --installed -v "${package}")"
...
check_rpm_specs() {
  local spec candidate expected epoch version actual found installed_names
  ...
  while IFS= read -r spec; do
    ...
    while [[ "${candidate}" == *-* ]]; do
      ...
      if grep -Fxq "${candidate}" <<<"${installed_names}"; then
...
check_tool_versions() {
  ...
  if [[ -n "${AICAGE_PACKAGE_TINI_INSTALL:-}" && "${AICAGE_PACKAGE_TINI_INSTALL}" == v* ]]; then
    tini_version="${AICAGE_PACKAGE_TINI_INSTALL#v}"
