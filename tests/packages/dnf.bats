...
@test "dnf resolver returns Fedora package versions as JSON" {
  ...
  [ "$(jq -r '.items[0].versions[0] | startswith("bash=")' <<<"${output}")" = "true" ]
}
...
@test "dnf resolver returns group members as JSON" {
  ...
  [ "$(jq -r '.items[1].versions[0] | startswith("7zip=")' <<<"${output}")" = "true" ]
  [ "$(jq -r '.items[2].name' <<<"${output}")" = "development-tools" ]
  [ "$(jq -r '.items[2].versions | length' <<<"${output}")" -eq 2 ]
  [ "$(jq -r '.items[2].versions[0] | startswith("gcc=")' <<<"${output}")" = "true" ]
  [ "$(jq -r '.items[2].versions[1] | startswith("make=")' <<<"${output}")" = "true" ]
}
