...
if command -v apk >/dev/null 2>&1; then
  ...
  if [[ -n "${AICAGE_PACKAGE_COREPACK_INSTALL:-}" ]]; then
    corepack_package="corepack@${AICAGE_PACKAGE_COREPACK_INSTALL}"
  fi
...
if ! command -v xdg-open >/dev/null 2>&1; then
  if command -v apt-get >/dev/null 2>&1; then
    ...
    dnf install -y "${AICAGE_PACKAGE_XDG_UTILS_INSTALL:-xdg-utils}"
  elif command -v yum >/dev/null 2>&1; then
    yum install -y "${AICAGE_PACKAGE_XDG_UTILS_INSTALL:-xdg-utils}"
